...
#Carga de datasets
normal_A=read.table('data/prostate-rsem-fpkm-gtex.txt.gz',header = T) #tejido de próstata
normal_B=read.table('data/lung-rsem-fpkm-gtex.txt.gz',header = T) #tejido de pulmones


tumor_A=read.table('data/prad-rsem-fpkm-tcga-t.txt.gz',header = T) #prostate adenocarcinoma
tumor_B=read.table('data/luad-rsem-fpkm-tcga-t.txt.gz',header = T) #lung adenocarcinoma 
```
...
Para reducir la dimensionalidad de los datos, vamos a realizar un filtrado de los genes, quedándonos solo con aquellos que están comprobados que son cancerígenos.

```{r}
#load cancer genes dataset
...
# Se excluyen las dos primeras filas del dataset tumor_A (nombres de genes e IDs de genes) 
...
```{r}
#Se reproduce lo anterior ahora con los datos de validación (test)
...
En primer lugar vamos a crear una nueva variable que combine ambas matrices para posteriormente poder manejarlo más facilmente.
...
```{r}
library(moments)  # Cargar el paquete moments 
...
Si el coeficiente de asimetría de una distribución está cerca de cero, significa que la distribución es aproximadamente simétrica. Una media cercana a cero indica que la distribución está balanceada alrededor de su centro. Por otro lado, si el coeficiente de asimetría es significativamente diferente de cero (positivo o negativo), indica que la distribución es asimétrica.

En el histograma, se puede observar la dispersión de los coeficientes de asimetría y su relación con la media. La mayoría de los coeficientes están cerca de cero, con una pequeña dispersión alrededor de la media, lo que significa que las distribuciones son, en su mayoría, simétricas. Si hubiera una dispersión mayor y los coeficientes tienden a alejarse de cero, podría indicar que hay asimetría en las distribuciones de los datos.

La cola de la distribución se extiende hacia la derecha, lo que indica que hay un número de variables con asimetría positiva significativa. Estos valores altos pueden significar la presencia de outliers o variables con distribuciones sesgadas.

### Porcentaje a superar por el modelo

Para considerar útil el mdoelo, debería presentar un porcentaje de aciertos mayor en la clasificación que al realizarlo por azar. Para comprobarlo, se calcula el porcentaje que se obtendría si se clasificaran todas las muestras como la clase mayoritaria (Tumor en nuestro caso). Si el modelo supera este porcentaje, puede ser tomado en cuenta para futuros estudios.
...
La población compuesta exclusivamente por muestras clasificadas como "normal" en el gráfico sugiere que estas muestras tienen características distintivas que las separan claramente de las muestras clasificadas como "tumor". Esto podría indicar la presencia de un subgrupo homogéneo de muestras normales en los datos.

Las dos poblaciones restantes, que muestran una mezcla de muestras clasificadas como "normal" y "tumor", sugieren cierto grado de solapamiento entre estas dos clases. Esto podría reflejar la variabilidad biológica o la heterogeneidad dentro de las muestras tumorales, donde algunas muestras comparten características similares a las muestras normales.

La presencia de múltiples poblaciones y cierto grado de solapamiento entre ellas indica una estructura de datos compleja. Esta complejidad podría estar relacionada con la diversidad biológica de las muestras, la presencia de diferentes subtipos de tumores o la variabilidad en la expresión génica entre las muestras.
...
Ahora comenzamos el entrenamiento de los algoritmos para la clasificación.
...
A continuación, haremos uso del parámetro selectionFunction = "oneSE" para que el árbol que se consiga sea de menor complejidad, haciendo que la posibilidad de que se produzca un sobreajuste sea menor.

```{r}
fit.cart2 <- train(Tipo~., data=train_set, method="rpart", metric="Accuracy", trControl=trainControl(method = "cv", number = 10, selectionFunction = "oneSE"), tuneLength = 10)
...
```{r}
res3 <- as_tibble(fit.svm$results[which.min(fit.svm$results[,2]),])
plot(fit.svm)
```

```{r}
df<-tibble(res3$Accuracy)
df
```
...
Para conocer cuáles son las variables a las que el modelo les ha dado más importancia a la hora de clasificar las muestras utilizaremos la función varImp() de caret.
...
Al analizar ambos gráficos se observa que hay genes que aparecen tanto en un modelo como en el otro. Es el caso de genes como FOXO1, el cual aparece en segundo lugar en ambos gráficos, aunque en el modelo SVM posee una mayor importancia; SIX2, el cual aparece en primer lugar para XGBoost y en quinto lugar para SVM; NOTCH1, apareciendo en el cuarto lugar en el gr ́afico de SVM y en séptimo lugar para XGBoost; STAT5B, que aparece en el tercer puesto para SVM y en el sexto para XGBoost. Por ultimo, RSPO3, el cual aparece en lugares inferiores para ambos algoritmos.
...
<!-- El siguiente paso es crear un explainer utilizando DALEX. Este explainer es un objeto que contiene el modelo, los datos, la variable objetivo y una etiqueta para identificar el modelo. Gracias a este objeto, DALEX es capaz de interpretar y explicar el modelo. -->

```{r}

#MODELO DE XGBOOST
# set.seed(7)
# model_xgb <- train(as.factor(Tipo) ~ ., data = train_setModelos, method = "xgbTree", metric="Accuracy", trControl = trainControl(method = "cv", number = 10), tuneLength = 1)
# 
